...
class cardShow1 extends StatefulWidget {
  const cardShow1({super.key});
  @override
  State<cardShow1> createState() => _cardShowState();
}

class _cardShowState extends State<cardShow1> with SingleTickerProviderStateMixin {
  //logic//
  ...
  getData() async {
    FirebaseFirestore.instance.collection('places').where("category" , isEqualTo:2).get().then((value) {
      setState(() {
        data = value.docs;
      });
    });
